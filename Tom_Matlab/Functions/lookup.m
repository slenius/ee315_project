...
% intersects at the desired points. Ig GM_ID is passed as the third
% argument, special care ios taken to look only "to the right" of the
...
p.addParameter('VGS', defaultVGS);
p.addParameter('VDS', defaultVDS);
p.addParameter('VSB', defaultVSB);
    ...
    for i = 1:dim(2)
        for j = 1:length(xdesired)
            ...
            if strcmp(numerator,'GM') && strcmp(denominator,'ID')
                [m, idx] = max(x(:, i));
                if max( xdesired(j)) > m
                    disp('lookup: GM_ID input larger than maximum!')
                    output = [];
                    return;
                else 
                    x_right = x(idx:end, i);
                ...
                if crossings > 1
                    ...
                    disp('*** Example: lookup(nch, ''ID_W'', ''GM_CGG'', gm_cgg, ''VGS'', nch.VGS(10:end))');
                    disp(' ')
                    figure(1000)
